...
class GatherersParallelStreamParallelGathererTest {
    ...
    static final Predicate<Integer> greaterThanZero = x -> {
        System.out.println(constructMessage("in 'x>0' filter for x=" + x));
        return x > 0;
    };

    static final Function<Integer, Integer> multiplyByTwo = x -> {
        System.out.println(constructMessage("in 'x*2' map for x=" + x));
        return x * 2;
    };

    static final Function<Integer, Integer> multiplyByThree = x -> {
        System.out.println(constructMessage("in 'x*3' map for x=" + x));
        return x * 3;
    };

    static final Predicate<Integer> lessThanTen = x -> {
        System.out.println(constructMessage("in 'x<10' filter for x=" + x));
        return x < 10;
    };

    static final Predicate<Integer> lessThanHundred = x -> {
     ...
     */
    static Gatherer<Integer, Void, Integer> onlyIntegratorSequenceGatherer = Gatherer.of((Void _, Integer x, Gatherer.Downstream<? super Integer> result) -> {
     ...
     */
    static Gatherer<Integer, Void, Integer> integratorWithFinisherSequenceGatherer = Gatherer.of((Void _, Integer x, Gatherer.Downstream<? super Integer> result) -> {
        System.out.println(constructMessage("in gatherer with only integrator for x=" + x));
        return result.push(x);
    ...
    */
    ...
    void sequenceStreamWithSequenceGatherer1() {
        ...
        var output1 = inputList.parallelStream().filter(greaterThanZero).map(multiplyByTwo).gather(onlyIntegratorSequenceGatherer).map(multiplyByThree).filter(lessThanHundred).limit(3).toList();
    ...
    void sequenceStreamWithSequenceGatherer2() {
        ...
        var output2 = inputList.parallelStream().filter(greaterThanZero).map(multiplyByTwo).gather(integratorWithFinisherSequenceGatherer).map(multiplyByThree).filter(lessThanHundred).limit(3).toList();
    ...
    void sequenceStreamWithSequenceGatherer3() {
        ...
        var output4 = inputList.parallelStream().filter(greaterThanZero).map(multiplyByTwo).gather(initializerWithIntegratorAndCombinerAndFinisherSequenceGatherer).map(multiplyByThree).filter(lessThanHundred).limit(3).toList();
    ...
    void sequenceStreamWithSequenceGatherer4() {
        ...
        var output5 = inputList.parallelStream().filter(greaterThanZero).map(multiplyByTwo).gather(initializerWithIntegratorAndCombinerAndFinisherSequenceGatherer_v2).map(multiplyByThree).filter(lessThanHundred).limit(3).toList();
